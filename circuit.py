...
# Set up the env
folder_path = '/data/leey19/sandbox/MOA_rade/Ketamine_E-I-main/05_Computational_Modeling/Ketamine_PV_10/'
...
TESTING = True #True # i.e.g generate 1 cell/pop, with 0.1 s runtime
...
if TESTING:
	OUTPUTPATH = f'Circuit_output_testing_raw/{GLOBALSEED}_{RANK}'
	N_HL23PN = 1
	N_HL23MN = 1
	N_HL23BN = 1
	N_HL23VN = 1
	...
	OUTPUTPATH = f'{folder_path}results_raw/{GLOBALSEED}_{RANK}'
...
simargs = {'rec_imem': False,
			   ...
			   'to_memory': True,
			   'to_file': True,
			   'file_name':f'OUTPUT_Seed{GLOBALSEED}.h5',
			   'dotprodcoeffs': None}
...
if RANK == 0:
    ...
    # ---------- SPIKES ----------------------------------------------------
    # Convert list-of-arrays to an object-dtype ndarray so np.save can pickle it
    spike_arr = np.asarray(SPIKES, dtype=object)
    np.save(os.path.join(OUTPUTPATH, f'SPIKES_Seed{GLOBALSEED}.npy'),spike_arr)

    # ---------- LFP -------------------------------------------------------
    if rec_LFP:
        np.save(os.path.join(OUTPUTPATH, f'OUTPUT_Seed{GLOBALSEED}.npy'), OUTPUT)

    # ---------- Current-dipole moment ------------------------------------
    if rec_DIPOLES:
        dpl_obj = np.asarray(DIPOLEMOMENT, dtype=object)
        np.save(os.path.join(OUTPUTPATH, f'DIPOLEMOMENT_Seed{GLOBALSEED}.npy'),dpl_obj)
